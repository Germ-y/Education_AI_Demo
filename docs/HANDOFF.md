...
- ElevenLabs 안내 음성은 선생님이 옆에서 말해주는 듯한 부드러운 톤을 목표로 한다. 기본값은 `ELEVENLABS_MODEL_ID=eleven_v3`이며, v3 사용 시 짧은 교사 톤 오디오 태그와 `stability/style/speed/speaker_boost` 설정을 함께 적용한다. 기본 속도는 느리게 들리지 않도록 `ELEVENLABS_SPEED=1.03`을 사용한다.
