...
| 학생등록 UX | 35% | 백엔드 등록과 NEIS 학교검색은 됨. 프론트 입력/검색/강점/약점/메모리 연결이 남음. |
...
- 프론트 `학생 등록` 버튼은 아직 실제 등록 UX로 닫혀 있지 않다.

해야 할 일:

- 학생등록 모달 추가.
- 학교명 입력 시 NEIS 학교검색 호출.
- 검색 결과에서 학교 선택.
- 이름, 학년, 반, 학생 유형, 현재 목표, 관찰 메모, 강점, 약점, 선호 지원 입력.
- 등록 성공 후 학생 목록/상세/자료 생성 탭 갱신.
- 새 학생에게 발급된 access code를 교사가 확인할 수 있게 표시.

완료 기준:

- 교사가 신규 학생을 등록하면 대시보드 목록에 바로 표시된다.
- 등록 학생의 `context-bundle`이 생성 요청에 바로 사용된다.
- 등록 학생도 기존 3명과 같은 콘텐츠 생성 플로우를 탄다.
...
1. `feat : 학생등록 프론트 연결`
2. `feat : asset 생성 job 상태 분리`
3. `fix : 콘텐츠 생성 stage draft 구조화`
4. `fix : 검토 preview 템플릿별 프레임 안정화`
5. `test : 교사 생성부터 학생 완료까지 e2e 추가`
6. `docs : 공유 DB와 asset 기준 갱신`
