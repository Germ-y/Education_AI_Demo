...
```text
1~3단계에서는 AI가 새로 분석하거나 새 콘텐츠를 즉석 생성하지 않는다.
4단계는 승인된 RealtimePracticeSpec 기반 실시간 연습이다.
AI는 수업 전 콘텐츠 패키지와 4단계 실시간 연습 스펙을 생성한다.
학생 화면은 승인된 콘텐츠 JSON과 승인된 RealtimePracticeSpec만 실행한다.
```

즉, 기본 인터랙션은 **사전에 생성되고 승인된 미션 콘텐츠**를 플레이하는 구조다. 다만 4단계는 `Realtime Practice`로 학생이 상황 이미지 안에서 AI와 짧게 대화하고 실시간 피드백을 받는다. 이때도 AI가 새 문제를 즉석 생성하는 것이 아니라, 교사가 승인한 상황/역할/루브릭 안에서만 대화한다.
...
3. 지금 해야 할 행동을 고름
4. AI가 정류장 직원 역할을 하고 학생이 직접 말로 도움을 요청함
```
...
4. 별이가 묻고, 학생이 마이크/텍스트로 "전체 4개 중 1개라서 1/4이야"라고 설명함
```
...
```text
생활지원형 또는 학습집중형 4단계 콘텐츠 골격을 만든다.
1~3단계는 정적 템플릿, 4단계는 승인형 RealtimePracticeSpec으로 만든다.
```
...
학습집중형 3단계: applied_question, mini_simulation, card_match 중 선택
각 유형 4단계: realtime_roleplay 또는 realtime_teach_back 고정
```
...
```text
4단계에서 학생이 상황 이미지와 AI 대화로 실제 말하기/설명하기를 연습한다.
```
...
단, 새 콘텐츠를 즉시 생성하지 않는다. 4단계 실시간 연습의 대화 기록은 루브릭 결과와 짧은 요약으로 압축해 다음 회기 근거로만 사용한다.
...
4. 콘텐츠 유형 결정
5. 4단계 StagePlan 생성
6. 1~3단계 템플릿 선택 및 4단계 RealtimePracticeSpec 생성
...
5. 4단계에서 승인된 realtime session 생성
...
4단계 realtime은 승인된 역할/루브릭/시간 제한 안에서만 실행한다.
...
MVP/공모전 데모에서는 학생과 선생님 데이터를 미리 넣어둔다. 회원가입/아이등록은 핵심 플로우가 안정된 뒤 시간이 남으면 추가한다.

### 1차

```text
seed 학생/교사/사례 데이터 적재
seed 계정 로그인
학생/사례 조회
메모리 카드 조회/수정
교사 회기 기록
