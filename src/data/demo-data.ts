import type { DemoDatabase } from "../domain/models.js";
import { MissionContentSchema } from "../domain/schemas.js";

const now = "2026-05-02T00:00:00.000Z";

const fractionContent = MissionContentSchema.parse({
  id: "content_fraction_001",
  caseId: "case_learning_fraction",
  studentId: "student_learning_fraction",
  contentType: "learning_focus",
  title: "분수 탐험: 빛나는 한 조각",
  sessionGoal: "전체 4개 중 1개를 1/4로 표현하고 말로 설명한다.",
  status: "published",
  totalSteps: 4,
  briefJson: {
    achievementStandard: "전체와 부분의 관계를 분수로 표현할 수 있다.",
    strategy: "시각 자료와 짧은 설명, 4단계 teach-back realtime",
  },
  teacherReviewSummary: "분모/분자 위치 혼동을 줄이기 위해 전체 수를 먼저 세는 흐름입니다.",
  approvedByUserId: "user_teacher_demo",
  approvedAt: now,
  publishedAt: now,
  stages: [
    {
      id: "stage_fraction_1",
      missionContentId: "content_fraction_001",
      step: 1,
      stageRole: "concept_intro",
      templateType: "concept_intro",
      studentTitle: "개념 열기",
      studentInstruction: "피자 지도를 보며 전체와 부분을 확인해요.",
      sortOrder: 1,
      templateJson: {
        imageAssetId: "asset_fraction_stage_1",
        storyText: "피자 한 판이 같은 크기 4조각으로 나뉘어 있어요.",
        missionText: "빛나는 조각이 전체 중 얼마인지 찾아봐요.",
      },
    },
    {
      id: "stage_fraction_2",
      missionContentId: "content_fraction_001",
      step: 2,
      stageRole: "basic_problem",
      templateType: "partition_picker",
      studentTitle: "문제 1",
      studentInstruction: "전체 조각 수와 고른 조각 수를 차례대로 세어보세요.",
      sortOrder: 2,
      templateJson: {
        imageAssetId: "asset_fraction_stage_2",
        question: "전체는 몇 조각인가요?",
        choices: [
          { id: "a", text: "1조각" },
          { id: "b", text: "4조각" },
          { id: "c", text: "2조각" },
        ],
        answer: "b",
        correctFeedback: "맞아요. 전체는 4조각이에요.",
        wrongFeedback: "피자 한 판이 몇 칸으로 나뉘었는지 다시 세어볼까요?",
      },
    },
    {
      id: "stage_fraction_3",
      missionContentId: "content_fraction_001",
      step: 3,
      stageRole: "applied_problem",
      templateType: "blank_fill",
      studentTitle: "문제 2",
      studentInstruction: "고른 조각 수와 전체 조각 수를 분수 자리에 넣어보세요.",
      sortOrder: 3,
      templateJson: {
        imageAssetId: "asset_fraction_stage_3",
        question: "전체 4개 중 1개는 __ / __ 이에요.",
        blanks: [
          { id: "numerator", answer: "1" },
          { id: "denominator", answer: "4" },
        ],
        acceptedAnswers: [{ numerator: "1", denominator: "4" }],
        correctFeedback: "좋아요. 위에는 고른 것 1, 아래에는 전체 4가 와요.",
        wrongFeedback: "위에는 고른 조각 수, 아래에는 전체 조각 수를 넣어요.",
      },
    },
    {
      id: "stage_fraction_4",
      missionContentId: "content_fraction_001",
      step: 4,
      stageRole: "realtime_practice",
      templateType: "realtime_teach_back",
      studentTitle: "AI에게 말해보기",
      studentInstruction: "별이에게 왜 1/4인지 말로 설명해보세요.",
      sortOrder: 4,
      templateJson: {
        imageAssetId: "asset_fraction_stage_4",
      },
      realtimeSpec: {
        id: "rt_spec_fraction_001",
        stageId: "stage_fraction_4",
        templateType: "realtime_teach_back",
        imageAssetId: "asset_fraction_stage_4",
        mode: "voice_or_text",
        practiceTitle: "별이에게 분수 설명하기",
        situationText: "별이가 빛나는 피자 조각을 보고 왜 1/4인지 궁금해해요.",
        aiRole: "별이",
        openingLine: "왜 4/1이 아니라 1/4인지 알려줄래?",
        studentGoal: "전체 4개 중 고른 것이 1개라서 1/4이라고 설명하기",
        rubric: [
          { id: "mention_whole", label: "전체가 4개임을 말한다", required: true },
          { id: "mention_part", label: "고른 것이 1개임을 말한다", required: true },
          { id: "connect_fraction", label: "1/4로 연결한다", required: true },
        ],
        allowedFeedback: [
          "좋아요. 전체가 몇 개인지 말했어요.",
          "고른 것이 몇 개인지도 말해볼까요?",
          "이제 1/4이라는 표현까지 이어서 말해봐요.",
        ],
        forbidden: ["새 문제를 만들지 않기", "학생에게 진단 라벨 말하지 않기", "개인정보 언급하지 않기"],
        maxTurns: 6,
        maxDurationSec: 120,
        postPracticeReflection: ["쉬웠어요", "조금 헷갈렸어요", "다시 연습하고 싶어요"],
      },
    },
  ],
  assets: [
    {
      id: "asset_fraction_hero",
      missionContentId: "content_fraction_001",
      assetRole: "hero",
      assetType: "image",
      provider: "openai",
      model: "gpt-image-2",
      storageUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      previewUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      qaStatus: "passed",
      approvalStatus: "approved",
    },
    {
      id: "asset_fraction_stage_1",
      missionContentId: "content_fraction_001",
      stageId: "stage_fraction_1",
      assetRole: "stage_1",
      assetType: "image",
      provider: "openai",
      model: "gpt-image-2",
      storageUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      previewUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      qaStatus: "passed",
      approvalStatus: "approved",
    },
    {
      id: "asset_fraction_stage_2",
      missionContentId: "content_fraction_001",
      stageId: "stage_fraction_2",
      assetRole: "stage_2",
      assetType: "image",
      provider: "openai",
      model: "gpt-image-2",
      storageUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      previewUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      qaStatus: "passed",
      approvalStatus: "approved",
    },
    {
      id: "asset_fraction_stage_3",
      missionContentId: "content_fraction_001",
      stageId: "stage_fraction_3",
      assetRole: "stage_3",
      assetType: "image",
      provider: "openai",
      model: "gpt-image-2",
      storageUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      previewUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      qaStatus: "passed",
      approvalStatus: "approved",
    },
    {
      id: "asset_fraction_stage_4",
      missionContentId: "content_fraction_001",
      stageId: "stage_fraction_4",
      assetRole: "stage_4_realtime",
      assetType: "image",
      provider: "openai",
      model: "gpt-image-2",
      storageUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      previewUrl: "/examples/generated/fraction-mission/fraction-pizza.png",
      qaStatus: "passed",
      approvalStatus: "approved",
    },
  ],
});

const busContent = MissionContentSchema.parse({
  id: "content_bus_001",
  caseId: "case_life_bus",
  studentId: "student_life_bus",
  contentType: "life_support",
  title: "센터 가는 길: 버스 타기",
  sessionGoal: "버스를 타고 센터에 갈 때 필요한 단서와 도움 요청 문장을 연습한다.",
  status: "published",
  totalSteps: 4,
  briefJson: {
    strategy: "생활 장면 이미지, 단서 찾기, 행동 선택, 4단계 roleplay realtime",
  },
  teacherReviewSummary: "버스 번호/정류장/도움 요청을 차례대로 확인하는 생활지원형 미션입니다.",
  approvedByUserId: "user_teacher_demo",
  approvedAt: now,
  publishedAt: now,
  stages: [
    {
      id: "stage_bus_1",
      missionContentId: "content_bus_001",
      step: 1,
      stageRole: "scenario_intro",
      templateType: "scenario_intro",
      studentTitle: "상황 만나기",
      studentInstruction: "센터에 가야 하는 상황을 살펴보세요.",
      sortOrder: 1,
      templateJson: {
        imageAssetId: "asset_bus_stage_1",
        storyText: "수민이가 우리 동네 센터에 가려고 버스 정류장에 왔어요.",
        missionText: "무엇을 먼저 확인해야 할까요?",
      },
    },
    {
      id: "stage_bus_2",
      missionContentId: "content_bus_001",
      step: 2,
      stageRole: "clue_identification",
      templateType: "scene_observation",
      studentTitle: "단서 찾기",
      studentInstruction: "버스를 타기 전에 중요한 단서를 골라보세요.",
      sortOrder: 2,
      templateJson: {
        imageAssetId: "asset_bus_stage_2",
        question: "센터에 가려면 무엇을 먼저 확인하면 좋을까요?",
        choices: [
          { id: "a", text: "버스 번호" },
          { id: "b", text: "하늘 색" },
          { id: "c", text: "가방 색" },
        ],
        answer: "a",
        correctFeedback: "좋아요. 버스 번호를 먼저 확인해요.",
        wrongFeedback: "센터로 가는 버스인지 알려주는 단서를 찾아볼까요?",
      },
    },
    {
      id: "stage_bus_3",
      missionContentId: "content_bus_001",
      step: 3,
      stageRole: "action_selection",
      templateType: "sequence_ordering",
      studentTitle: "행동 고르기",
      studentInstruction: "센터에 가는 순서를 차례대로 골라보세요.",
      sortOrder: 3,
      templateJson: {
        imageAssetId: "asset_bus_stage_3",
        question: "알맞은 순서는 무엇일까요?",
        cards: ["버스 번호 확인", "버스 타기", "센터 도착"],
        answer: ["버스 번호 확인", "버스 타기", "센터 도착"],
        correctFeedback: "좋아요. 먼저 확인하고, 그다음 버스를 타요.",
        wrongFeedback: "버스를 타기 전에 확인해야 할 일을 앞으로 옮겨볼까요?",
      },
    },
    {
      id: "stage_bus_4",
      missionContentId: "content_bus_001",
      step: 4,
      stageRole: "realtime_practice",
      templateType: "realtime_roleplay",
      studentTitle: "AI와 연습하기",
      studentInstruction: "AI 안내 직원에게 센터 가는 길을 물어보세요.",
      sortOrder: 4,
      templateJson: {
        imageAssetId: "asset_bus_stage_4",
      },
      realtimeSpec: {
        id: "rt_spec_bus_001",
        stageId: "stage_bus_4",
        templateType: "realtime_roleplay",
        imageAssetId: "asset_bus_stage_4",
        mode: "voice_or_text",
        practiceTitle: "정류장에서 도움 요청하기",
        situationText: "학생이 센터에 가려고 버스 정류장에서 안내 직원에게 도움을 요청합니다.",
        aiRole: "정류장 안내 직원",
        openingLine: "어디로 가려고 하나요? 제가 도와줄게요.",
        studentGoal: "센터에 가야 한다고 말하고 어떤 버스를 타야 하는지 도움 요청하기",
        rubric: [
          { id: "state_destination", label: "목적지를 말한다", required: true },
          { id: "ask_help", label: "도움을 요청한다", required: true },
          { id: "confirm_next_action", label: "다음 행동을 확인한다", required: true },
        ],
        allowedFeedback: [
          "좋아요. 어디로 가는지 말했어요.",
          "이제 어떤 도움을 받고 싶은지도 말해볼까요?",
          "마지막으로 다음에 무엇을 할지 확인해봐요.",
        ],
        forbidden: ["새로운 이동 문제 만들지 않기", "학생 개인정보 묻지 않기", "불안감을 키우는 표현 금지"],
        maxTurns: 6,
        maxDurationSec: 120,
        postPracticeReflection: ["할 수 있을 것 같아요", "조금 떨려요", "다시 연습하고 싶어요"],
      },
    },
  ],
  assets: ["hero", "stage_1", "stage_2", "stage_3", "stage_4_realtime"].map((assetRole, index) => ({
    id: `asset_bus_${assetRole}`,
    missionContentId: "content_bus_001",
    stageId: index === 0 ? undefined : `stage_bus_${index}`,
    assetRole,
    assetType: "image",
    provider: "openai",
    model: "gpt-image-2",
    storageUrl: "/assets/demo/bus-mission-placeholder.png",
    previewUrl: "/assets/demo/bus-mission-placeholder.png",
    qaStatus: "passed",
    approvalStatus: "approved",
  })),
});

export function createDemoDatabase(): DemoDatabase {
  return {
    organizations: [
      {
        id: "org_yeongju_center",
        externalKey: "demo_org_yeongju_center",
        name: "영주 기초학력거점지원센터",
        type: "learning_support_center",
        regionCode: "47210",
      },
    ],
    users: [
      {
        id: "user_admin_demo",
        organizationId: "org_yeongju_center",
        email: "admin.demo@eduyj.local",
        displayName: "데모 관리자",
        role: "center_admin",
        status: "active",
      },
      {
        id: "user_teacher_demo",
        organizationId: "org_yeongju_center",
        email: "teacher.demo@eduyj.local",
        displayName: "데모 선생님",
        role: "teacher",
        status: "active",
      },
      {
        id: "user_reviewer_demo",
        organizationId: "org_yeongju_center",
        email: "reviewer.demo@eduyj.local",
        displayName: "콘텐츠 검수자",
        role: "content_reviewer",
        status: "active",
      },
    ],
    students: [
      {
        id: "student_learning_fraction",
        organizationId: "org_yeongju_center",
        externalKey: "demo_student_learning_fraction",
        displayName: "민준",
        grade: "middle_2",
        schoolCode: "demo_middle_school",
        studentType: "learning_focus",
        primaryNeed: "분수의 전체-부분 관계 이해",
        profileJson: {
          interests: ["요리", "탐험", "게임형 보상"],
          readingLoad: "low",
          choiceCountLimit: 3,
        },
        status: "active",
      },
      {
        id: "student_life_bus",
        organizationId: "org_yeongju_center",
        externalKey: "demo_student_life_bus",
        displayName: "수민",
        grade: "elementary_6",
        schoolCode: "demo_elementary_school",
        studentType: "life_support",
        primaryNeed: "센터 이동 순서와 도움 요청 연습",
        profileJson: {
          interests: ["동네 지도", "역할극"],
          readingLoad: "very_low",
          choiceCountLimit: 2,
        },
        status: "active",
      },
    ],
    studentAccounts: [
      { id: "student_account_learning", studentId: "student_learning_fraction", accessCode: "STAR-001", status: "active" },
      { id: "student_account_life", studentId: "student_life_bus", accessCode: "STAR-002", status: "active" },
    ],
    supportCases: [
      {
        id: "case_learning_fraction",
        studentId: "student_learning_fraction",
        ownerTeacherId: "user_teacher_demo",
        caseStatus: "open",
        currentGoal: "분수의 전체/부분 관계를 시각 자료로 안정화",
        openedAt: now,
      },
      {
        id: "case_life_bus",
        studentId: "student_life_bus",
        ownerTeacherId: "user_teacher_demo",
        caseStatus: "open",
        currentGoal: "센터 이동 상황에서 단서 확인과 도움 요청 연습",
        openedAt: now,
      },
    ],
    caseNotes: [
      {
        id: "note_fraction_001",
        caseId: "case_learning_fraction",
        authorId: "user_teacher_demo",
        noteType: "session",
        body: "긴 설명보다 그림을 먼저 보여주면 집중이 좋아짐. 4/1 선택 경험 있음.",
        visibility: "teacher_only",
        createdAt: now,
      },
      {
        id: "note_bus_001",
        caseId: "case_life_bus",
        authorId: "user_teacher_demo",
        noteType: "consultation",
        body: "새로운 장소에 갈 때 순서를 놓치면 불안해함. 도움 요청 문장 연습 필요.",
        visibility: "teacher_only",
        createdAt: now,
      },
    ],
    memoryCards: [
      {
        id: "memory_fraction_active",
        studentId: "student_learning_fraction",
        caseId: "case_learning_fraction",
        version: 1,
        learningProblemTypes: ["concept_misunderstanding", "numerator_denominator_confusion", "low_confidence"],
        recent4wResponseJson: {
          summary: "시각 자료 반응은 좋으나 분모/분자 위치 혼동이 반복됨",
          accuracyTrend: [0.4, 0.5, 0.5, 0.75],
        },
        emotionalStateNote: "실패 경험 후 회피가 조금 있음. 첫 문항 성공 경험이 필요함.",
        effectiveExplanationStyles: ["visual_example", "short_steps", "mascot_teach_back"],
        frequentBlockingUnits: ["fractions", "word_problem_conditions"],
        guardianCooperationStatus: "normal",
        nextSessionCautions: ["전체 수를 먼저 세고 고른 수를 나중에 세기", "긴 문장 설명 피하기"],
        teacherVerifiedAt: now,
        status: "active",
      },
      {
        id: "memory_bus_active",
        studentId: "student_life_bus",
        caseId: "case_life_bus",
        version: 1,
        learningProblemTypes: ["sequence_planning", "help_request_avoidance", "situation_clue_miss"],
        recent4wResponseJson: {
          summary: "시각 순서 카드에는 잘 반응하지만 실제 말하기에서 멈춤이 있음",
          completionTrend: [0.5, 0.7, 0.8, 0.8],
        },
        emotionalStateNote: "처음 보는 상황에서 긴장함. 역할극 전 이미지 예고가 효과적임.",
        effectiveExplanationStyles: ["scenario_image", "two_choice", "roleplay"],
        frequentBlockingUnits: ["daily_route", "asking_help"],
        guardianCooperationStatus: "needs_followup",
        nextSessionCautions: ["도움 요청 문장을 먼저 연습", "선택지는 2개 위주로 제공"],
        teacherVerifiedAt: now,
        status: "active",
      },
    ],
    plannerItems: [
      {
        id: "planner_fraction_next",
        studentId: "student_learning_fraction",
        caseId: "case_learning_fraction",
        periodType: "next_session",
        goalText: "1/4 설명을 성공한 뒤 2/4로 확장",
        checklistJson: { checks: ["첫 문제 성공경험", "분모/분자 위치 재확인"] },
        status: "planned",
      },
      {
        id: "planner_bus_next",
        studentId: "student_life_bus",
        caseId: "case_life_bus",
        periodType: "next_session",
        goalText: "버스 정류장 도움 요청 문장 1개 말하기",
        checklistJson: { checks: ["목적지 말하기", "도움 요청하기", "다음 행동 확인"] },
        status: "planned",
      },
    ],
    missionContents: [fractionContent, busContent],
    attempts: [],
    activityEvents: [],
    realtimeSessions: [],
    publicDataSources: [
      {
        id: "source_neis",
        sourceCode: "neis_open_api",
        name: "나이스 교육정보 개방 포털",
        baseUrl: "https://open.neis.go.kr/",
        authType: "api_key",
        enabled: false,
      },
      {
        id: "source_schoolinfo",
        sourceCode: "schoolinfo_api",
        name: "학교알리미 Open API",
        baseUrl: "https://www.schoolinfo.go.kr/",
        authType: "api_key",
        enabled: false,
      },
      {
        id: "source_curriculum_seed",
        sourceCode: "curriculum_seed",
        name: "교육과정 성취기준 seed",
        authType: "manual_seed",
        enabled: true,
      },
    ],
  };
}
