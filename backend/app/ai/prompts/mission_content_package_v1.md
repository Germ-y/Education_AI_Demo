...
정답은 스키마 모양만 맞으면 되는 값이 아닙니다. 각 문제를 먼저 직접 풀어 보고, `answer`, 정답 선택지 문구, `correctFeedback`, `wrongFeedback`, `fixedLine`, `acceptedAnswers`, `matches`, `answerOrder`가 모두 같은 풀이 사실을 말하게 합니다.

- 수량, 계산, 비율, 조건 비교, 분류 기준 문제는 최종 JSON을 쓰기 전에 조용히 한 번 검산합니다.
- 나눗셈의 몫과 나머지를 다루면 `나누는 수 × 몫 + 나머지 = 전체 수`이고, 나머지는 나누는 수보다 작아야 합니다.
- 비례·비율·시간·길이·개수 문제는 선택한 정답 문구와 피드백의 숫자 관계가 서로 모순되면 실패입니다.
- `wrong_explanation_fix`에서는 `wrongLine`만 틀린 설명이어야 합니다. `fixedLine`은 실제 정답 설명이고, `answer`가 가리키는 선택지와 같은 뜻이어야 합니다.
- 정답 선택지는 `fixedLine`의 표현을 그대로 복사하지 않아도 되지만, 같은 계산 결과와 같은 개념 판단을 말해야 합니다.
- 오답 선택지는 그럴듯하되, 정답 피드백이나 `fixedLine`과 같은 뜻이면 안 됩니다.
...
- `answerOrder`는 `cards`가 적힌 배열 순서가 아니라, 질문이 요구한 의미 순서입니다. 예를 들어 “더 단단한 것부터”라면 가장 단단한 카드 id가 첫 번째여야 합니다.
