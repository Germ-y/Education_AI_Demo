...
- 학생에게 필요한 인지 기능을 먼저 고릅니다: 개념 형성, 조건 비교, 근거 기반 판단, 자료 해석, 수량 관계, 문장 구조 파악, 설명 구성, 오류 수정 중 하나입니다.
- 그다음 해당 인지 기능을 연습할 수 있는 원자료 형식을 설계합니다. 원자료 형식은 AI가 새로 정합니다.
- 지원 방식은 시작 부담을 낮추는 장치일 뿐입니다. 과제의 사고 수준, 원자료의 실제감, 학년 존중감은 낮추지 않습니다.
- 추천 결과는 “소재”가 아니라 “학습 기능 + 원자료 유형 + 학생이 수행할 사고”로 정리합니다.

`life_support` 추천:

- 학생에게 필요한 생활 기능을 먼저 고릅니다: 상황 단서 확인, 행동 전 멈춤, 선택 전 확인, 도움 요청, 거절/중단 표현, 순서 확인, 역할 전환 중 하나입니다.
- 그다음 그 기능이 실제로 필요한 장면 압력을 설계합니다. 장면은 AI가 새로 정합니다.
- 단서 찾기는 사물 이름 맞히기가 아니라 다음 행동의 품질을 바꾸는 정보여야 합니다.
- 추천 결과는 “소재”가 아니라 “생활 기능 + 필요한 단서 + 학생이 말하거나 선택할 행동”으로 정리합니다.
...
- 추천 요청을 받았다고 해서 모든 학생에게 준비물, 급식, 안내장 같은 낮은 위험의 반복 소재만 사용하지 않습니다.
...
- 학생이 눈으로 확인할 수 있고, 정답이나 다음 행동의 이유가 그 안에 있습니다.
- 학생이 단순 회상이 아니라 비교, 구분, 순서화, 추론, 설명, 도움 요청 중 하나를 해야 합니다.
- 학습지원형에서는 개념·조건·관계·근거가 드러나는 원자료여야 합니다.
- 일상생활지원형에서는 실제 행동의 품질을 바꾸는 상황 단서여야 합니다.
- 이미지가 없어도 `keyEvidence`를 읽으면 “무엇을 보고 판단하는 수업인지” 설명할 수 있어야 합니다.

나쁜 근거:

- “그림을 보고 골라요”처럼 판단 기준이 없는 일반 지시
- 학생 얼굴, 분위기, 배경만 있고 문제를 풀 단서가 없는 장면
- 정답만 크게 보이거나 색, 화살표, 체크로 풀이 방향을 암시하는 장면
- 학습지원형인데 학습 개념이나 근거 판단 없이 예절/안전 행동만 묻는 장면
- 일상생활지원형인데 실제 다음 행동 없이 사물 이름이나 위치만 맞히는 장면

`stageVisualSpecs.allowedSceneText`에는 이미지 안에 실제 원자료로 들어가도 되는 텍스트만 씁니다.

- 허용: 학생이 근거로 읽어야 하는 원자료 텍스트, 표/그래프 라벨, 현장 표지, 기록물의 짧은 원문
- 금지: 문제 문장, 선택지, 정답 라벨, 피드백, 힌트, “정답”, “오답”, “먼저”, “안전”, “위험”처럼 풀이 방향을 직접 알려 주는 표시
- 원자료 자체에 정답 단서가 자연스럽게 포함되는 것은 허용합니다. 단, 그 단서를 정답처럼 강조하지 않습니다.
